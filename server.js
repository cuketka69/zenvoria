...
function sanitizeGuideArticleBody(value) {
  return sanitizeHtml(String(value || '').slice(0, GUIDE_ARTICLE_BODY_MAX_CHARS), {
    ...
    allowedClasses: {
      div: ['guide-callout', 'guide-callout-tip', 'guide-callout-important', 'guide-callout-warn', 'guide-checklist', 'guide-image-gallery', 'guide-image-placeholder', 'guide-alternating-section', 'guide-alternating-copy', 'guide-alternating-recommendation', 'guide-alternating-conclusion'],
      p: ['guide-source'],
...
function sanitizeGuideArticleRevisions(value) {
  ...
  for (const raw of value.slice(-5)) {
    ...
    out.push({ title, author, category, lead, body, template: raw.template === 'alternating' ? 'alternating' : 'classic', time: guideReadingTimeLabel(raw.time), featured: raw.featured === true, published: raw.published !== false, createdAt, savedBy });
  }
  return out;
}
function sanitizeGuideArticles(value) {
  ...
  for (const raw of value) {
    ...
    out.push({
      slug, title, author, lead, body, category, image: imageData ? imageData.dataUrl : '', template: raw.template === 'alternating' ? 'alternating' : 'classic',
...
function recurringBookingMail({ user, caregiverName, service, time, created, skipped, country }) {
  const sk = country === 'sk';
  const firstName = (user.name || '').trim().split(/\s+/)[0] || (sk ? 'zákazník' : 'zákazníku');
  ...
  if (sk) {
    const facts = [
      ...
      { label: 'Opatrovateľka', value: caregiverName || '' },
      { label: 'Počet vytvorených termínov', value: String(created.length) },
    ];
    if (skipped.length) facts.push({ label: 'Vynechané termíny', value: skipped.map((s) => s.date).join(', ') });
    return {
      subject: `Opakovaná objednávka vytvorená (${created.length} termínov)`,
      text:
        `Dobrý deň, ${user.name},\n\n` +
        `vytvorili sme vám ${created.length} opakovaných objednávok u opatrovateľky ${caregiverName || ''} (${service}, ${time}):\n` +
        `${datesText}\n` +
        (skipped.length ? `\nNiektoré termíny sa nepodarilo vytvoriť (opatrovateľka ich má obsadené alebo blokované): ${skipped.map((s) => s.date).join(', ')}\n` : '') +
        '\nKaždú objednávku musí opatrovateľka zvlášť potvrdiť.\n\n' +
        'S pozdravom,\nTím ZENVORIA',
      html: renderEmailLayout({
        preheader: `Vytvorili sme ${created.length} opakovaných objednávok.`,
        title: 'Opakovaná objednávka vytvorená',
        intro: `Dobrý deň, ${firstName}. Vytvorili sme vám ${created.length} opakovaných objednávok u opatrovateľky ${caregiverName || ''}.`,
        bodyHtml:
          `<p style="margin:0 0 10px 0;"><b>Termíny:</b> ${escapeHtml(datesText)}</p>` +
          (skipped.length ? `<p style="margin:0 0 10px 0;color:#B23A2E;"><b>Nepodarilo sa vytvoriť:</b> ${escapeHtml(skipped.map((s) => s.date).join(', '))} (opatrovateľka ich má obsadené alebo blokované)</p>` : '') +
          '<p style="margin:0;">Každú objednávku musí opatrovateľka zvlášť potvrdiť, rovnako ako pri jednorazovej rezervácii.</p>',
        ctaLabel: 'Zobraziť moje objednávky',
        ctaUrl: `${APP_URL_SK}/#bookings`,
        ctaNote: '',
        facts,
        closingTitle: 'Ďakujeme za vašu dôveru.',
        closingSubtitle: 'Tím Zenvoria',
        footerNote: 'Tento e-mail slúži ako potvrdenie práve vytvorenej opakovanej objednávky v ZENVORIA.',
      }),
    };
  }
  const facts = [
...
function changeEmailLinkMail({ user, confirmUrl, country }) {
  const sk = country === 'sk';
  const firstName = (user.name || '').trim().split(/\s+/)[0] || (sk ? 'zákazník' : 'zákazníku');
  if (sk) {
    return {
      subject: 'Potvrdenie zmeny e-mailu v ZENVORIA',
      text:
        `Dobrý deň, ${user.name || firstName},\n\n` +
        'dostali sme žiadosť o zmenu e-mailovej adresy pri vašom účte ZENVORIA.\n\n' +
        `Pre pokračovanie otvorte tento odkaz: ${confirmUrl}\n\n` +
        'Odkaz je platný 30 minút. Ak ste o zmenu e-mailu nežiadali, tento e-mail ignorujte.\n\n' +
        'S pozdravom,\nTím ZENVORIA',
      html: renderEmailLayout({
        preheader: 'Potvrďte zmenu e-mailu bezpečným odkazom na pôvodnej adrese.',
        title: 'Zmena e-mailu',
        intro: `Dobrý deň, ${firstName}. Na zmenu e-mailovej adresy najprv potrebujeme potvrdiť prístup k vášmu súčasnému e-mailu.`,
        bodyHtml:
          '<p style="margin:0 0 14px 0;">Kliknutím na tlačidlo nižšie otvoríte bezpečnú stránku, kde zadáte nový e-mail. Naň vám následne pošleme overovací kód.</p>' +
          '<p style="margin:0;">Ak ste o zmenu nežiadali, nič sa nestane a tento e-mail môžete ignorovať.</p>',
        ctaLabel: 'Potvrdiť zmenu e-mailu',
        ctaUrl: confirmUrl,
        ctaNote: `Odkaz je platný 30 minút. Ak tlačidlo nefunguje, otvorte tento odkaz: ${confirmUrl}`,
        facts: [
          { label: 'Súčasný e-mail', value: user.email || '' },
          { label: 'Typ požiadavky', value: 'Zmena e-mailu' },
          { label: 'Platnosť odkazu', value: '30 minút' },
        ],
        closingTitle: 'Bezpečnosť je pre nás prioritou.',
        closingSubtitle: 'Tím Zenvoria',
        footerNote: 'Tento e-mail bol odoslaný automaticky po žiadosti o zmenu e-mailu v ZENVORIA.',
      }),
    };
  }
  return {
...
function changeEmailCodeMail({ user, newEmail, code, country }) {
  const sk = country === 'sk';
  const firstName = (user.name || '').trim().split(/\s+/)[0] || (sk ? 'zákazník' : 'zákazníku');
  if (sk) {
    return {
      subject: 'Overovací kód pre nový e-mail v ZENVORIA',
      text:
        `Dobrý deň, ${user.name || firstName},\n\n` +
        `na potvrdenie novej e-mailovej adresy ${newEmail} zadajte tento kód: ${code}\n\n` +
        'Kód je platný 10 minút. Ak ste o zmenu e-mailu nežiadali, tento e-mail ignorujte.\n\n' +
        'S pozdravom,\nTím ZENVORIA',
      html: renderEmailLayout({
        preheader: 'Posielame vám overovací kód pre novú e-mailovú adresu.',
        title: 'Overenie nového e-mailu',
        intro: `Dobrý deň, ${firstName}. Na dokončenie zmeny e-mailu zadajte do aplikácie tento šesťmiestny kód.`,
        bodyHtml:
          ...
          '<p style="margin:0;">Kód je platný 10 minút. Ak ste o zmenu e-mailu nežiadali, tento e-mail ignorujte.</p>',
        ctaLabel: 'Otvoriť ZENVORIA',
        ctaUrl: `${APP_URL_SK}/#settings`,
        ...
        facts: [
          ...
          { label: 'Platnosť kódu', value: '10 minút' },
        ],
        closingTitle: 'Ďakujeme za potvrdenie.',
        closingSubtitle: 'Tím Zenvoria',
        footerNote: 'Tento e-mail bol odoslaný automaticky pri overení novej e-mailovej adresy v ZENVORIA.',
      }),
    };
  }
  return {
...
function serviceDoneCheckMail({ familyName, order, caregiverName, country }) {
  const sk = country === 'sk';
  const firstName = (familyName || '').trim().split(/\s+/)[0] || (sk ? 'zákazník' : 'zákazníku');
  const when = [order.date, order.time].filter(Boolean).join(sk ? ' o ' : ' v ');
  if (sk) {
    return {
      subject: `Prebehla starostlivosť ${order.date}? Potvrďte prosím dokončenie`,
      text:
        `Dobrý deň, ${familyName || firstName},\n\n` +
        `naplánovaný čas služby (${order.service}, ${when}) u opatrovateľky ${caregiverName || ''} už uplynul.\n` +
        'Ak všetko prebehlo v poriadku, označte prosím objednávku vo svojom účte ako dokončenú — odomkne sa vám tým možnosť opatrovateľku ohodnotiť.\n\n' +
        'S pozdravom,\nTím ZENVORIA',
      html: renderEmailLayout({
        preheader: 'Potvrďte prosím, že starostlivosť prebehla.',
        title: 'Prebehla starostlivosť v poriadku?',
        intro: `Dobrý deň, ${firstName}. Naplánovaný čas služby u opatrovateľky ${caregiverName || ''} už uplynul.`,
        bodyHtml:
          '<p style="margin:0 0 14px 0;">Ak všetko prebehlo v poriadku, označte prosím objednávku vo svojom účte ako dokončenú.</p>' +
          '<p style="margin:0;">Odomkne sa vám tým možnosť opatrovateľku ohodnotiť.</p>',
        ...
        closingTitle: 'Ďakujeme za využitie ZENVORIA.',
        closingSubtitle: 'Tím Zenvoria',
        footerNote: 'Tento e-mail sa posiela jedenkrát, krátko po uplynutí naplánovaného konca služby.',
      }),
    };
  }
  return {
...
function upcomingOrderReminderMail({ name, order, counterpartName, forCaregiver, country }) {
  const sk = country === 'sk';
  const firstName = (name || '').trim().split(/\s+/)[0] || (forCaregiver ? (sk ? 'opatrovateľka' : 'pečovatelko') : (sk ? 'zákazník' : 'zákazníku'));
  const when = [order.date, order.time].filter(Boolean).join(sk ? ' o ' : ' v ');
  if (sk) {
    const counterpartLabel = forCaregiver ? 'Klient' : 'Opatrovateľka';
    return {
      subject: `Pripomienka: zajtra máte naplánovanú starostlivosť (${order.date})`,
      text:
        `Dobrý deň, ${name || firstName},\n\n` +
        `pripomíname naplánovanú službu: ${order.service}, ${when}.\n` +
        `${counterpartLabel}: ${counterpartName || ''}\n\n` +
        'S pozdravom,\nTím ZENVORIA',
      html: renderEmailLayout({
        preheader: 'Zajtra máte naplánovanú starostlivosť.',
        title: 'Pripomienka nadchádzajúcej služby',
        intro: `Dobrý deň, ${firstName}. Pripomíname vašu naplánovanú službu.`,
        bodyHtml: '<p style="margin:0;">Ak sa termín z nejakého dôvodu zmenil, ozvite sa prosím druhej strane cez chat čo najskôr.</p>',
        ctaLabel: forCaregiver ? 'Zobraziť kalendár' : 'Zobraziť objednávky',
        ctaUrl: forCaregiver ? `${APP_URL_SK}/#cg-calendar` : `${APP_URL_SK}/#bookings`,
        ctaNote: '',
        facts: [
          { label: 'Služba', value: order.service || '' },
          { label: 'Termín', value: when || '' },
        ...
        closingTitle: 'Tešíme sa, nech všetko prebehne v poriadku.',
        closingSubtitle: 'Tím Zenvoria',
        footerNote: 'Tento e-mail sa posiela jedenkrát, približne 24 hodín pred naplánovaným začiatkom služby.',
      }),
    };
  }
...
function termDecisionMail({ name, accepted, term, country }) {
  const sk = country === 'sk';
  const firstName = (name || '').trim().split(/\s+/)[0] || '';
  const when = [term && term.date, term && term.time].filter(Boolean).join(sk ? ' o ' : ' v ');
  if (sk) {
    return {
      subject: accepted ? `Váš navrhnutý termín bol prijatý (${when})` : `Váš navrhnutý termín bol odmietnutý (${when})`,
      text:
        `Dobrý deň, ${name || firstName},\n\n` +
        (accepted
          ? `váš navrhnutý termín (${when}) bol prijatý.\n`
          : `váš navrhnutý termín (${when}) bol bohužiaľ odmietnutý.\n`) +
        'S pozdravom,\nTím ZENVORIA',
      html: renderEmailLayout({
        preheader: accepted ? 'Váš navrhnutý termín bol prijatý.' : 'Váš navrhnutý termín bol odmietnutý.',
        title: accepted ? 'Termín prijatý' : 'Termín odmietnutý',
        intro: `Dobrý deň, ${firstName}. ${accepted ? 'Váš navrhnutý termín bol prijatý.' : 'Váš navrhnutý termín bol bohužiaľ odmietnutý.'}`,
        bodyHtml: accepted
          ? '<p style="margin:0;">Detaily nájdete vo svojom účte v sekcii Správy alebo Objednávky.</p>'
          : '<p style="margin:0;">Skúste v chate navrhnúť iný termín.</p>',
        ctaLabel: 'Otvoriť chat',
        ctaUrl: `${APP_URL_SK}/#chat`,
        ctaNote: '',
        facts: [{ label: 'Navrhnutý termín', value: when || '' }],
        closingTitle: accepted ? 'Tešíme sa, nech všetko prebehne v poriadku.' : 'Sme tu pre vás.',
        closingSubtitle: 'Tím Zenvoria',
        footerNote: 'Tento e-mail informuje o vybavení vášho návrhu termínu v chate.',
      }),
    };
  }
  return {
...
function broadcastMail({ name, text, country }) {
  const sk = country === 'sk';
  const firstName = (name || '').trim().split(/\s+/)[0] || '';
  if (sk) {
    return {
      subject: 'Správa od tímu ZENVORIA',
      text: `Dobrý deň, ${name || firstName},\n\n${text}\n\nS pozdravom,\nTím ZENVORIA`,
      html: renderEmailLayout({
        preheader: text.slice(0, 120),
        title: 'Správa od tímu ZENVORIA',
        intro: `Dobrý deň, ${firstName}.`,
        bodyHtml: `<p style="margin:0;white-space:pre-wrap;">${escapeHtml(text)}</p>`,
        ctaLabel: 'Otvoriť ZENVORIA',
        ctaUrl: APP_URL_SK,
        ctaNote: '',
        facts: [],
        closingTitle: 'Ďakujeme, že ste s nami.',
        closingSubtitle: 'Tím Zenvoria',
        footerNote: 'Túto správu odoslal správca systému ZENVORIA vašej skupine používateľov.',
      }),
    };
  }
  return {
...
function favoriteAvailableMail({ familyName, caregiverName, country }) {
  const sk = country === 'sk';
  const firstName = (familyName || '').trim().split(/\s+/)[0] || (sk ? 'zákazník' : 'zákazníku');
  if (sk) {
    return {
      subject: `${caregiverName} je opäť dostupná na ZENVORIA`,
      text:
        `Dobrý deň, ${familyName || firstName},\n\n` +
        `vaša obľúbená opatrovateľka ${caregiverName} je znovu dostupná pre nové objednávky.\n\n` +
        'S pozdravom,\nTím ZENVORIA',
      html: renderEmailLayout({
        preheader: `${caregiverName} je opäť dostupná.`,
        title: 'Obľúbená opatrovateľka je späť',
        intro: `Dobrý deň, ${firstName}. Opatrovateľka ${caregiverName}, ktorú máte v obľúbených, je znovu dostupná pre nové objednávky.`,
        bodyHtml: '<p style="margin:0;">Môžete jej rovno poslať dopyt.</p>',
        ctaLabel: 'Zobraziť profil',
        ctaUrl: `${APP_URL_SK}/#search`,
        ctaNote: '',
        facts: [{ label: 'Opatrovateľka', value: caregiverName }],
        closingTitle: 'Ďakujeme, že ste s nami.',
        closingSubtitle: 'Tím Zenvoria',
        footerNote: 'Tento e-mail ste dostali, pretože máte túto opatrovateľku uloženú v obľúbených.',
      }),
    };
  }
  return {
...
function certExpiryReminderMail({ name, certs, country }) {
  const sk = country === 'sk';
  const firstName = (name || '').trim().split(/\s+/)[0] || (sk ? 'opatrovateľka' : 'pečovatelko');
  if (sk) {
    const list = certs.map((c) => `${c.name || 'Osvedčenie'} (platnosť do ${c.validUntil})`);
    return {
      subject: certs.length > 1 ? 'Čoskoro vyprší platnosť vašich osvedčení' : 'Čoskoro vyprší platnosť vášho osvedčenia',
      text:
        `Dobrý deň, ${name || firstName},\n\n` +
        `blíži sa koniec platnosti:\n${list.map((l) => '- ' + l).join('\n')}\n\n` +
        'Nahrajte prosím aktuálne osvedčenie cez formulár overenia, aby vám profil zostal dôveryhodný.\n\n' +
        'S pozdravom,\nTím ZENVORIA',
      html: renderEmailLayout({
        preheader: 'Blíži sa koniec platnosti vášho osvedčenia.',
        title: 'Platnosť osvedčenia čoskoro vyprší',
        intro: `Dobrý deň, ${firstName}. Blíži sa koniec platnosti pri týchto osvedčeniach:`,
        bodyHtml: `<ul style="margin:0;padding-left:18px;">${list.map((l) => `<li style="margin-bottom:6px;">${escapeHtml(l)}</li>`).join('')}</ul>`,
        ctaLabel: 'Nahrať nové osvedčenie',
        ctaUrl: `${APP_URL_SK}/#cg-verify`,
        ctaNote: '',
        facts: [],
        closingTitle: 'Vďaka, že si udržujete profil aktuálny.',
        closingSubtitle: 'Tím Zenvoria',
        footerNote: 'Tento e-mail sa posiela jedenkrát, približne 30 dní pred vypršaním platnosti osvedčenia.',
      }),
    };
  }
...
function verificationResultMail({ name, approved, reason, country }) {
  const sk = country === 'sk';
  const firstName = (name || '').trim().split(/\s+/)[0] || (sk ? 'opatrovateľka' : 'pečovatelko');
  if (sk) {
    return {
      subject: approved ? 'Vaše overenie bolo schválené' : 'Vaša žiadosť o overenie bola zamietnutá',
      text:
        `Dobrý deň, ${name || firstName},\n\n` +
        (approved
          ? 'gratulujeme — vaša žiadosť o overenie bola schválená. Váš profil je teraz viditeľný rodinám.\n'
          : `vašu žiadosť o overenie sme bohužiaľ nemohli schváliť.\n${reason ? 'Dôvod: ' + reason + '\n' : ''}Upravte prosím údaje a odošlite znova.\n`) +
        '\nS pozdravom,\nTím ZENVORIA',
      html: renderEmailLayout({
        preheader: approved ? 'Vaše overenie bolo schválené.' : 'Vaša žiadosť o overenie bola zamietnutá.',
        title: approved ? 'Overenie schválené' : 'Žiadosť zamietnutá',
        intro: approved
          ? `Gratulujeme, ${firstName}. Vaša žiadosť o overenie bola schválená.`
          : `Dobrý deň, ${firstName}. Vašu žiadosť o overenie sme bohužiaľ nemohli schváliť.`,
        bodyHtml: approved
          ? '<p style="margin:0;">Váš profil je teraz <b>viditeľný rodinám</b> vo vyhľadávaní. Doplňte si ponuku služieb a dostupnosť v kalendári, aby vás oslovilo čo najviac rodín.</p>'
          : `<p style="margin:0 0 14px 0;">${reason ? 'Dôvod: <b>' + escapeHtml(reason) + '</b>' : 'Niektoré údaje nebolo možné overiť.'}</p><p style="margin:0;">Upravte prosím svoje údaje či doklady a odošlite žiadosť znova.</p>`,
        ctaLabel: approved ? 'Otvoriť profil' : 'Upraviť a odoslať znova',
        ctaUrl: approved ? `${APP_URL_SK}/#cg-profile` : `${APP_URL_SK}/#cg-verify`,
        ctaNote: '',
        facts: [{ label: 'Stav overenia', value: approved ? 'Schválené' : 'Zamietnuté' }],
        closingTitle: approved ? 'Vitajte medzi overenými opatrovateľkami.' : 'Radi vám pomôžeme.',
        closingSubtitle: 'Tím Zenvoria',
        footerNote: 'Tento e-mail informuje o výsledku vašej žiadosti o overenie v ZENVORIA.',
      }),
    };
  }
  return {
...
const HOME_SK_TRANSLATIONS = [
  ...
  // view-legal (shell — samotný obsah dokumentů je v app.js, viz LEGAL objekt)
  ['<span id="legalBackLabel">Zpět</span>', '<span id="legalBackLabel">Späť</span>'],
  ['<div class="legal-badge">Právní dokument</div>', '<div class="legal-badge">Právny dokument</div>'],
  ['<div class="legal-lead" id="legalLead">Právní informace k používání platformy ZENVORIA najdete přehledně na jednom místě.</div>',
    '<div class="legal-lead" id="legalLead">Právne informácie o používaní platformy ZENVORIA nájdete prehľadne na jednom mieste.</div>'],
  ['<div class="legal-note">Právní centrum je už veřejně přístupné a sdílitelná URL funguje. Samotné znění dokumentů před ostrým provozem potvrďte finální verzí schválenou pro vaše podnikání.</div>',
    '<div class="legal-note">Právne centrum je už verejne prístupné a zdieľateľná URL funguje. Samotné znenie dokumentov pred ostrou prevádzkou potvrďte finálnou verziou schválenou pre vaše podnikanie.</div>'],
  ['<div class="legal-kicker">Přímý odkaz</div>', '<div class="legal-kicker">Priamy odkaz</div>'],
  ['<button type="button" class="btn btn-ghost" onclick="copyLegalLink()">Kopírovat odkaz</button>', '<button type="button" class="btn btn-ghost" onclick="copyLegalLink()">Kopírovať odkaz</button>'],
  ['<button type="button" class="btn btn-ghost" onclick="window.print()">Tisknout</button>', '<button type="button" class="btn btn-ghost" onclick="window.print()">Tlačiť</button>'],
  ['<div class="legal-kicker">Provozovatel</div>', '<div class="legal-kicker">Prevádzkovateľ</div>'],
  ['<span id="legalCompanyMeta">IČO doplňte před ostrým spuštěním.</span>', '<span id="legalCompanyMeta">IČO doplňte pred ostrým spustením.</span>'],
  ['<button type="button" class="btn btn-gold" onclick="closeLegal()">Rozumím, zpět</button>', '<button type="button" class="btn btn-gold" onclick="closeLegal()">Rozumiem, späť</button>'],
  ['<button type="button" class="btn btn-ghost" onclick="go(\'home\')">Na úvodní stránku</button>', '<button type="button" class="btn btn-ghost" onclick="go(\'home\')">Na úvodnú stránku</button>'],
];
...
const JS_SK_TRANSLATIONS = [
  ...
  ["${planPrice(key)>0?fmtMoney(planPrice(key))+' <span>/ měsíc</span>':'Zdarma'}", "${planPrice(key)>0?fmtMoney(planPrice(key))+' <span>/ mesiac</span>':'Zdarma'}"],
  ...
  ["confirmLabel:'Nahlásit',onConfirm:(reason)=>{\r\n      reason=(reason||'').trim();\r\n      if(reason.length<5){toast('Popište prosím stručně důvod nahlášení.','declined');return;}\r\n      apiSync(api('/conversations/'+c.id+'/messages/'+mid+'/report',{method:'POST',body:{reason}}).then(()=>{",
    "confirmLabel:'Nahlásiť',onConfirm:(reason)=>{\r\n      reason=(reason||'').trim();\r\n      if(reason.length<5){toast('Popíšte prosím stručne dôvod nahlásenia.','declined');return;}\r\n      apiSync(api('/conversations/'+c.id+'/messages/'+mid+'/report',{method:'POST',body:{reason}}).then(()=>{"],
  // LEGAL — obsah právních dokumentů (GDPR/podmínky/cookies) zobrazovaný v appce (view-legal).
  // Pracovní slovenský překlad s odkazy na slovenské instituce — PŘED ostrým nasazením pro SK
  // je potřeba nechat finální znění zkontrolovat slovenským právníkem (viz plán SK lokalizace).
  ["title:'Ochrana osobních údajů',upd:'Naposledy aktualizováno: 6. června 2026',lead:'Jak chráníme osobní údaje rodin, pečovatelek i dalších uživatelů platformy ZENVORIA.'",
    "title:'Ochrana osobných údajov',upd:'Naposledy aktualizované: 6. júna 2026',lead:'Ako chránime osobné údaje rodín, opatrovateliek aj ďalších používateľov platformy ZENVORIA.'"],
  ["<p>Ochrana vašich osobních údajů je pro ZENVORIA prioritou. Tento dokument popisuje, jaké údaje zpracováváme, za jakým účelem a jaká práva v souvislosti s nimi máte. Zpracování probíhá v souladu s nařízením (EU) 2016/679 (GDPR) a zákonem č. 110/2019 Sb.</p>",
    "<p>Ochrana vašich osobných údajov je pre ZENVORIA prioritou. Tento dokument popisuje, aké údaje spracúvame, na aký účel a aké práva v súvislosti s nimi máte. Spracovanie prebieha v súlade s nariadením (EÚ) 2016/679 (GDPR) a zákonom č. 18/2018 Z. z. o ochrane osobných údajov.</p>"],
  ["<h2>1. Správce údajů</h2>", "<h2>1. Prevádzkovateľ údajov</h2>"],
  ["<p>Správcem osobních údajů je ZENVORIA s.r.o., IČO 000 00 000, se sídlem v Praze. Kontakt: <a href=\"mailto:miklasova@zenvoria.cz\">miklasova@zenvoria.cz</a>.</p>",
    "<p>Prevádzkovateľom osobných údajov je ZENVORIA s.r.o., IČO 000 00 000, so sídlom v Bratislave. Kontakt: <a href=\"mailto:miklasova@zenvoria.cz\">miklasova@zenvoria.cz</a>.</p>"],
  ["<h2>2. Jaké údaje zpracováváme</h2>", "<h2>2. Aké údaje spracúvame</h2>"],
  ["<li>Identifikační a kontaktní údaje (jméno, e-mail, telefon, adresa).</li>", "<li>Identifikačné a kontaktné údaje (meno, e-mail, telefón, adresa).</li>"],
  ["<li>Údaje o objednávkách péče a komunikaci v aplikaci.</li>", "<li>Údaje o objednávkach starostlivosti a komunikácii v aplikácii.</li>"],
  ["<li>U pečovatelek doklady o ověření, certifikace a reference.</li>", "<li>Pri opatrovateľkách doklady o overení, certifikácie a referencie.</li>"],
  ["<li>Technické údaje (IP adresa, typ zařízení, soubory cookies).</li>", "<li>Technické údaje (IP adresa, typ zariadenia, súbory cookies).</li>"],
  ["<h2>3. Účel a právní základ zpracování</h2>", "<h2>3. Účel a právny základ spracovania</h2>"],
  ["<li>Poskytování služby a plnění smlouvy (čl. 6 odst. 1 písm. b GDPR).</li>", "<li>Poskytovanie služby a plnenie zmluvy (čl. 6 ods. 1 písm. b GDPR).</li>"],
  ["<li>Plnění zákonných povinností, např. účetnictví (písm. c).</li>", "<li>Plnenie zákonných povinností, napr. účtovníctvo (písm. c).</li>"],
  ["<li>Oprávněný zájem — bezpečnost a zlepšování služby (písm. f).</li>", "<li>Oprávnený záujem — bezpečnosť a zlepšovanie služby (písm. f).</li>"],
  ["<li>Marketing pouze na základě vašeho souhlasu (písm. a).</li>", "<li>Marketing len na základe vášho súhlasu (písm. a).</li>"],
  ["<h2>4. Doba uchování</h2>", "<h2>4. Doba uchovávania</h2>"],
  ["<p>Údaje uchováváme po dobu trvání smluvního vztahu a následně po dobu vyžadovanou právními předpisy (zpravidla 10 let u daňových dokladů).</p>",
    "<p>Údaje uchovávame počas trvania zmluvného vzťahu a následne po dobu vyžadovanú právnymi predpismi (spravidla 10 rokov pri daňových dokladoch).</p>"],
  ["<p>Máte právo na přístup k údajům, jejich opravu či výmaz, omezení zpracování, přenositelnost, vznesení námitky a podání stížnosti u Úřadu pro ochranu osobních údajů (<a href=\"https://www.uoou.cz\" target=\"_blank\" rel=\"noopener\">uoou.cz</a>).</p>",
    "<p>Máte právo na prístup k údajom, ich opravu či výmaz, obmedzenie spracovania, prenosnosť, vznesenie námietky a podanie sťažnosti na Úrade na ochranu osobných údajov SR (<a href=\"https://www.dataprotection.gov.sk\" target=\"_blank\" rel=\"noopener\">dataprotection.gov.sk</a>).</p>"],
  ["<p>S dotazy ke zpracování osobních údajů se obraťte na <a href=\"mailto:miklasova@zenvoria.cz\">miklasova@zenvoria.cz</a>.</p>",
    "<p>S otázkami k spracovaniu osobných údajov sa obráťte na <a href=\"mailto:miklasova@zenvoria.cz\">miklasova@zenvoria.cz</a>.</p>"],
  ["title:'Obchodní podmínky',upd:'Účinné od: 6. června 2026',lead:'Pravidla používání platformy ZENVORIA, registrace, objednávek péče a odpovědnosti jednotlivých stran.'",
    "title:'Obchodné podmienky',upd:'Účinné od: 6. júna 2026',lead:'Pravidlá používania platformy ZENVORIA, registrácie, objednávok starostlivosti a zodpovednosti jednotlivých strán.'"],
  ["<p>Tyto obchodní podmínky upravují práva a povinnosti při užívání platformy ZENVORIA, která zprostředkovává spojení mezi rodinami a ověřenými pečovatelkami.</p>",
    "<p>Tieto obchodné podmienky upravujú práva a povinnosti pri používaní platformy ZENVORIA, ktorá sprostredkúva spojenie medzi rodinami a overenými opatrovateľkami.</p>"],
  ["<h2>1. Úvodní ustanovení</h2>", "<h2>1. Úvodné ustanovenia</h2>"],
  ["<p>Provozovatelem platformy je ZENVORIA s.r.o. Registrací a užíváním služby vyjadřujete souhlas s těmito podmínkami.</p>",
    "<p>Prevádzkovateľom platformy je ZENVORIA s.r.o. Registráciou a používaním služby vyjadrujete súhlas s týmito podmienkami.</p>"],
  ["<h2>2. Vymezení pojmů</h2>", "<h2>2. Vymedzenie pojmov</h2>"],
  ["<li><b>Uživatel</b> — rodina nebo osoba poptávající péči.</li>", "<li><b>Používateľ</b> — rodina alebo osoba dopytujúca starostlivosť.</li>"],
  ["<li><b>Pečovatelka</b> — ověřený poskytovatel pečovatelských služeb.</li>", "<li><b>Opatrovateľka</b> — overený poskytovateľ opatrovateľských služieb.</li>"],
  ["<li><b>Zprostředkování</b> — propojení uživatele a pečovatelky prostřednictvím platformy.</li>", "<li><b>Sprostredkovanie</b> — prepojenie používateľa a opatrovateľky prostredníctvom platformy.</li>"],
  ["<h2>3. Registrace a účet</h2>", "<h2>3. Registrácia a účet</h2>"],
  ["<p>Uživatel je povinen uvádět pravdivé údaje a chránit přístupové údaje ke svému účtu. Za aktivity provedené pod účtem odpovídá jeho držitel.</p>",
    "<p>Používateľ je povinný uvádzať pravdivé údaje a chrániť prístupové údaje k svojmu účtu. Za aktivity vykonané pod účtom zodpovedá jeho držiteľ.</p>"],
  ["<h2>4. Zprostředkování péče</h2>", "<h2>4. Sprostredkovanie starostlivosti</h2>"],
  ["<p>ZENVORIA poskytuje technologickou platformu pro vyhledání a objednání péče. Samotnou péči poskytuje pečovatelka, která za ni nese odpovědnost. Pečovatelky procházejí ověřením dokladů a referencí.</p>",
    "<p>ZENVORIA poskytuje technologickú platformu na vyhľadanie a objednanie starostlivosti. Samotnú starostlivosť poskytuje opatrovateľka, ktorá za ňu nesie zodpovednosť. Opatrovateľky prechádzajú overením dokladov a referencií.</p>"],
  ["<p>Ceny péče jsou uvedeny u jednotlivých pečovatelek. Platby probíhají prostřednictvím zabezpečené platební brány. Platforma si může účtovat zprostředkovatelský poplatek.</p>",
    "<p>Ceny starostlivosti sú uvedené pri jednotlivých opatrovateľkách. Platby prebiehajú prostredníctvom zabezpečenej platobnej brány. Platforma si môže účtovať sprostredkovateľský poplatok.</p>"],
  ["<h2>6. Zrušení a storno</h2>", "<h2>6. Zrušenie a storno</h2>"],
  ["<p>Objednávku lze bezplatně zrušit nejpozději 24 hodin před začátkem péče. Při pozdějším zrušení může být účtován storno poplatek.</p>",
    "<p>Objednávku možno bezplatne zrušiť najneskôr 24 hodín pred začiatkom starostlivosti. Pri neskoršom zrušení môže byť účtovaný storno poplatok.</p>"],
  ["<h2>7. Odpovědnost a reklamace</h2>", "<h2>7. Zodpovednosť a reklamácie</h2>"],
  ["<p>Případné reklamace lze uplatnit na <a href=\"mailto:miklasova@zenvoria.cz\">miklasova@zenvoria.cz</a>. Spotřebitel má právo na mimosoudní řešení sporů u České obchodní inspekce.</p>",
    "<p>Prípadné reklamácie možno uplatniť na <a href=\"mailto:miklasova@zenvoria.cz\">miklasova@zenvoria.cz</a>. Spotrebiteľ má právo na mimosúdne riešenie sporov na Slovenskej obchodnej inšpekcii (<a href=\"https://www.soi.sk\" target=\"_blank\" rel=\"noopener\">soi.sk</a>).</p>"],
  ["<h2>8. Závěrečná ustanovení</h2>", "<h2>8. Záverečné ustanovenia</h2>"],
  ["<p>Tyto podmínky se řídí právním řádem České republiky. Provozovatel je oprávněn podmínky měnit; o změnách bude informovat v aplikaci.</p>",
    "<p>Tieto podmienky sa riadia právnym poriadkom Slovenskej republiky. Prevádzkovateľ je oprávnený podmienky meniť; o zmenách bude informovať v aplikácii.</p>"],
  ["title:'Zásady používání cookies',upd:'Naposledy aktualizováno: 6. června 2026',lead:'Přehled souborů cookies, které používáme pro fungování aplikace, nastavení a analytiku.'",
    "title:'Zásady používania cookies',upd:'Naposledy aktualizované: 6. júna 2026',lead:'Prehľad súborov cookies, ktoré používame na fungovanie aplikácie, nastavenia a analytiku.'"],
  ["<p>Soubory cookies nám pomáhají zajistit správné fungování aplikace a zlepšovat vaši zkušenost. Zde se dozvíte, jaké cookies používáme a jak je můžete spravovat.</p>",
    "<p>Súbory cookies nám pomáhajú zaistiť správne fungovanie aplikácie a zlepšovať vašu skúsenosť. Tu sa dozviete, aké cookies používame a ako ich môžete spravovať.</p>"],
  ["<h2>1. Co jsou cookies</h2>", "<h2>1. Čo sú cookies</h2>"],
  ["<p>Cookies jsou malé textové soubory ukládané ve vašem prohlížeči. Umožňují například zapamatovat přihlášení nebo předvolby zobrazení.</p>",
    "<p>Cookies sú malé textové súbory ukladané vo vašom prehliadači. Umožňujú napríklad zapamätať prihlásenie alebo predvoľby zobrazenia.</p>"],
  ["<h2>2. Jaké cookies používáme</h2>", "<h2>2. Aké cookies používame</h2>"],
  ["<li><b>Nezbytné</b> — zajišťují základní funkce, přihlášení a bezpečnost. Nelze je vypnout.</li>", "<li><b>Nevyhnutné</b> — zaisťujú základné funkcie, prihlásenie a bezpečnosť. Nemožno ich vypnúť.</li>"],
  ["<li><b>Předvolby</b> — pamatují si nastavení, např. světlý/tmavý režim.</li>", "<li><b>Predvoľby</b> — pamätajú si nastavenia, napr. svetlý/tmavý režim.</li>"],
  ["<li><b>Analytické</b> — anonymně měří návštěvnost a pomáhají službu zlepšovat.</li>", "<li><b>Analytické</b> — anonymne merajú návštevnosť a pomáhajú službu zlepšovať.</li>"],
  ["<p>Nastavení cookies můžete kdykoli změnit ve svém prohlížeči, kde je lze také smazat. Omezení některých cookies může ovlivnit funkčnost aplikace.</p>",
    "<p>Nastavenia cookies môžete kedykoľvek zmeniť vo svojom prehliadači, kde ich možno aj vymazať. Obmedzenie niektorých cookies môže ovplyvniť funkčnosť aplikácie.</p>"],
  ["<p>S dotazy ke cookies se obraťte na <a href=\"mailto:miklasova@zenvoria.cz\">miklasova@zenvoria.cz</a>.</p>",
    "<p>S otázkami ku cookies sa obráťte na <a href=\"mailto:miklasova@zenvoria.cz\">miklasova@zenvoria.cz</a>.</p>"],
  // openLegal — popisky kolem právního dokumentu (zpět na registraci/přihlášení, výchozí lead)
  ["document.getElementById('legalLead').textContent=d.lead||'Právní informace k používání platformy ZENVORIA najdete přehledně na jednom místě.';",
    "document.getElementById('legalLead').textContent=d.lead||'Právne informácie o používaní platformy ZENVORIA nájdete prehľadne na jednom mieste.';"],
  ["document.getElementById('legalBackLabel').textContent=legalBackView==='register'?'Zpět k registraci':(legalBackView==='login'?'Zpět k přihlášení':'Zpět');",
    "document.getElementById('legalBackLabel').textContent=legalBackView==='register'?'Späť k registrácii':(legalBackView==='login'?'Späť k prihláseniu':'Späť');"],
];
...
app.get('/api/locations/search', rateLimit('locations', RATE_LIMITS.locations), h(async (req, res) => {
  ...
  if (countryForReq(req) === 'sk') {
    try { return res.json({ items: await searchAddressesSk(q, 10) }); }
    catch (e) { console.warn('[locations] SK vyhledávání adres selhalo:', e.message); return res.json({ items: [] }); }
  }
...
app.get('/api/locations/search-municipality', rateLimit('locations', RATE_LIMITS.locations), h(async (req, res) => {
  ...
  if (!q || q.length < 2) return res.json({ items: [] });
  if (countryForReq(req) === 'sk') {
    try { return res.json({ items: await searchMunicipalitiesSk(q, 8) }); }
    catch (e) { console.warn('[locations] SK vyhledávání obcí selhalo:', e.message); return res.json({ items: [] }); }
  }
...
app.get('/api/locations/reverse', rateLimit('locations', RATE_LIMITS.locations), h(async (req, res) => {
  ...
  if (!Number.isFinite(lat) || !Number.isFinite(lng)) return res.status(400).json({ error: 'Neplatné souřadnice.' });
  if (countryForReq(req) === 'sk') {
    try { return res.json({ item: await reverseGeocodeSk(lat, lng) }); }
    catch (e) { console.warn('[locations] SK reverse geocoding selhal:', e.message); return res.json({ item: null }); }
  }
...
app.post('/api/auth/change-email/request', requireAuth, rateLimit('change-email-request', RATE_LIMITS.changeEmailRequest), h(async (req, res) => {
  ...
  const confirmUrl = `${appUrlFor(user.country)}/?changeEmail=${encodeURIComponent(token)}`;
  await sendMailSafe({ to: user.email, ...changeEmailLinkMail({ user, confirmUrl, country: user.country }) });
...
app.post('/api/auth/change-email/send-code', rateLimit('change-email-code', RATE_LIMITS.changeEmailCode), h(async (req, res) => {
  ...
  await sendMailSafe({ to: newEmail, ...changeEmailCodeMail({ user, newEmail, code, country: user.country }) });
...
app.get('/api/bootstrap', h(async (req, res) => {
  ...
  res.json({
    ...
    planPrices: sanitizePlanPrices(settings.planPrices) || { cz: { start: 190, premium: 390 }, sk: { start: 8, premium: 16 } },
...
app.post('/api/recurring-bookings', requireRole('family', 'admin'), requireVerifiedEmail, rateLimit('orders', RATE_LIMITS.orders), h(async (req, res) => {
  ...
  const summaryMail = recurringBookingMail({ user: req.session, caregiverName, service, time, created, skipped, country: countryForReq(req) });
...
app.post('/api/verifications/:id/approve', requireRole('admin'), h(async (req, res) => {
  ...
  if (v.email) await notifyMail({ to: v.email, category: 'email', ...verificationResultMail({ name: v.name, approved: true, country: userCountry || countryForReq(req) }) });
  if (v.email) {
...
app.post('/api/verifications/:id/reject', requireRole('admin'), h(async (req, res) => {
  ...
  const rejectUser = v && v.email ? await findUserByEmail(v.email) : null;
  if (v && v.email) await notifyMail({ to: v.email, category: 'email', ...verificationResultMail({ name: v.name, approved: false, reason, country: rejectUser && rejectUser.country }) });
  if (v && v.email) {
    const vu = rejectUser;
...
async function notifyFavoritersCaregiverAvailable(caregiverId, caregiverName) {
  try {
    ...
    for (const f of favRows) {
      ...
      await notifyMail({ to: f.family_email, settings: u && u.settings, category: 'email', ...favoriteAvailableMail({ familyName: u && u.name, caregiverName, country: u && u.country }) });
      if (u) {
...
async function notifyTermDecision(proposerId, accepted, term) {
  try {
    const rows = await restSelect(T.users, `id=eq.${encodeURIComponent(proposerId)}&select=name,email,settings,country&limit=1`);
    const u = rows && rows[0];
    if (!u || !u.email) return;
    await notifyMail({ to: u.email, settings: u.settings, category: 'chat', ...termDecisionMail({ name: u.name, accepted, term, country: u.country }) });
...
app.post('/api/broadcasts', requireRole('admin'), h(async (req, res) => {
  ...
  try {
    if (audience === 'specific') {
      const list = emails.map((e) => `"${e}"`).join(',');
      recipients = list ? (await restSelect(T.users, `email=in.(${list})&select=name,email,settings,country`)) || [] : [];
    } else {
      let filter = 'select=name,email,settings,country';
    ...
    for (const u of recipients) {
      if (u && u.email) notifyMail({ to: u.email, settings: u.settings, category: 'email', ...broadcastMail({ name: u.name, text, country: u.country }) });
    }
...
function sendNotFoundPage(req, res, { kind = 'page' } = {}) {
  ...
  const html = `<!doctype html><html lang="${sk ? 'sk' : 'cs'}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex"><title>${escapeHtml(title)} — ZENVORIA</title><style>
    *{box-sizing:border-box}html,body{margin:0;min-height:100%;font-family:Arial,sans-serif;background:#062b1b;color:#f7f4eb}body{min-height:100vh;display:flex;flex-direction:column}header{display:flex;height:82px;align-items:center;padding:0 clamp(24px,5vw,76px);border-bottom:1px solid rgba(201,162,51,.22);background:#082f1e}header a{display:inline-flex;align-items:center;gap:12px;color:#fff;font-size:21px;font-weight:800;letter-spacing:.08em;text-decoration:none}header img{width:42px;height:42px;object-fit:contain}.nf-main{position:relative;display:grid;flex:1;place-items:center;overflow:hidden;padding:56px 24px;background:radial-gradient(circle at 50% 42%,#0b4b2f 0,#062b1b 62%)}.nf-main:before,.nf-main:after{position:absolute;border:1px solid rgba(201,162,51,.28);border-radius:50%;content:""}.nf-main:before{width:440px;height:440px;top:-230px;right:-130px;box-shadow:0 0 0 65px rgba(201,162,51,.045)}.nf-main:after{width:300px;height:300px;bottom:-190px;left:-90px}.nf-card{position:relative;z-index:1;width:min(680px,100%);padding:clamp(35px,6vw,62px);border:1px solid rgba(201,162,51,.3);border-radius:28px;background:#0d462d;text-align:center;box-shadow:0 28px 70px rgba(0,0,0,.32)}.nf-code{display:inline-flex;align-items:center;gap:12px;color:#d8b443;font-size:13px;font-weight:800;letter-spacing:.16em;text-transform:uppercase}.nf-code:before,.nf-code:after{width:28px;height:1px;background:#c9a233;content:""}.nf-icon{display:grid;width:82px;height:82px;margin:24px auto;place-items:center;border-radius:50%;background:rgba(201,162,51,.12);color:#d8b443}.nf-card h1{margin:0;color:#fff;font-family:Georgia,serif;font-size:clamp(36px,7vw,55px);line-height:1.08}.nf-card p{max-width:520px;margin:18px auto 0;color:rgba(255,255,255,.7);font-size:17px;line-height:1.7}.nf-actions{display:flex;justify-content:center;gap:12px;flex-wrap:wrap;margin-top:30px}.nf-btn{display:inline-flex;min-height:48px;align-items:center;justify-content:center;padding:12px 20px;border:1px solid #c9a233;border-radius:12px;background:#c9a233;color:#082f1e;font-size:14px;font-weight:800;text-decoration:none}.nf-btn.secondary{border-color:rgba(201,162,51,.52);background:transparent;color:#fff}.nf-btn:hover{filter:brightness(1.08);transform:translateY(-1px)}@media(max-width:560px){header{height:70px}.nf-main{padding:30px 16px}.nf-card{padding:34px 22px;border-radius:22px}.nf-actions{flex-direction:column}.nf-btn{width:100%}}
...
// slovenský překlad statických právních stránek (přímé URL /obchodni-podminky apod., mimo appku) —
// stejný pracovní draft jako LEGAL v app.js, se slovenskými právními odkazy; před ostrým nasazením
// pro SK nechat zkontrolovat slovenským právníkem
const STATIC_LEGAL_SK_TRANSLATIONS = [
  ['<html lang="cs">', '<html lang="sk">'],
  // obchodni-udaje.html
  ['<title>Obchodní údaje | ZENVORIA</title>', '<title>Obchodné údaje | ZENVORIA</title>'],
  ['<meta name="description" content="Kontaktní a obchodní údaje společnosti ZENVORIA.">', '<meta name="description" content="Kontaktné a obchodné údaje spoločnosti ZENVORIA.">'],
  ['<div class="eyebrow">Kontakt</div>\n        <h1>Obchodní údaje</h1>\n        <p>Veřejně dostupné kontaktní a identifikační údaje provozovatele platformy ZENVORIA.</p>',
    '<div class="eyebrow">Kontakt</div>\n        <h1>Obchodné údaje</h1>\n        <p>Verejne dostupné kontaktné a identifikačné údaje prevádzkovateľa platformy ZENVORIA.</p>'],
  ['<h2>Provozovatel</h2>', '<h2>Prevádzkovateľ</h2>'],
  ['<h2>Kontaktní údaje</h2>', '<h2>Kontaktné údaje</h2>'],
  ['<p>E-mail: <a href="mailto:miklasova@zenvoria.cz">miklasova@zenvoria.cz</a><br>Telefon: <a href="tel:{{CONTACT_PHONE_TEL}}">{{CONTACT_PHONE}}</a><br>Web: <a href="https://www.zenvoria.cz">www.zenvoria.cz</a></p>',
    '<p>E-mail: <a href="mailto:miklasova@zenvoria.cz">miklasova@zenvoria.cz</a><br>Telefón: <a href="tel:{{CONTACT_PHONE_TEL}}">{{CONTACT_PHONE}}</a><br>Web: <a href="https://www.zenvoria.sk">www.zenvoria.sk</a></p>'],
  ['<h2>Identifikační údaje</h2>', '<h2>Identifikačné údaje</h2>'],
  ['<p>IČO: {{CONTACT_ICO}}<br>Sídlo: {{CONTACT_ADDRESS}}</p>', '<p>IČO: {{CONTACT_ICO}}<br>Sídlo: {{CONTACT_ADDRESS}}</p>'],
  ['<h2>Reklamace a komunikace</h2>', '<h2>Reklamácie a komunikácia</h2>'],
  ['<p>Veškeré dotazy, reklamace nebo žádosti směřujte na <a href="mailto:miklasova@zenvoria.cz">miklasova@zenvoria.cz</a>.</p>',
    '<p>Všetky otázky, reklamácie alebo žiadosti smerujte na <a href="mailto:miklasova@zenvoria.cz">miklasova@zenvoria.cz</a>.</p>'],
  ['<h3>Rychlý kontakt</h3>', '<h3>Rýchly kontakt</h3>'],
  ['<h3>Právní dokumenty</h3>', '<h3>Právne dokumenty</h3>'],
  // společné pro všechny 4 stránky
  ['<a class="brand" href="/"><img src="/logo.webp" alt="ZENVORIA"><strong>ZENVORIA</strong></a>',
    '<a class="brand" href="/"><img src="/logo.webp" alt="ZENVORIA"><strong>ZENVORIA</strong></a>'],
  ['aria-label="Právní navigace"', 'aria-label="Právna navigácia"'],
  ['<a href="/obchodni-podminky">Obchodní podmínky</a>', '<a href="/obchodni-podminky">Obchodné podmienky</a>'],
  ['<a class="active" href="/obchodni-podminky">Obchodní podmínky</a>', '<a class="active" href="/obchodni-podminky">Obchodné podmienky</a>'],
  ['<a href="/ochrana-osobnich-udaju">Ochrana osobních údajů</a>', '<a href="/ochrana-osobnich-udaju">Ochrana osobných údajov</a>'],
  ['<a class="active" href="/ochrana-osobnich-udaju">Ochrana osobních údajů</a>', '<a class="active" href="/ochrana-osobnich-udaju">Ochrana osobných údajov</a>'],
  ['<a href="/zasady-cookies">Zásady cookies</a>', '<a href="/zasady-cookies">Zásady cookies</a>'],
  ['<a class="active" href="/zasady-cookies">Zásady cookies</a>', '<a class="active" href="/zasady-cookies">Zásady cookies</a>'],
  ['<a href="/obchodni-udaje">Obchodní údaje</a>', '<a href="/obchodni-udaje">Obchodné údaje</a>'],
  ['<a class="active" href="/obchodni-udaje">Obchodní údaje</a>', '<a class="active" href="/obchodni-udaje">Obchodné údaje</a>'],
  ['<a class="btn" href="/obchodni-podminky">Podmínky</a>', '<a class="btn" href="/obchodni-podminky">Podmienky</a>'],
  ['<a class="btn" href="/obchodni-udaje">Obchodní údaje</a>', '<a class="btn" href="/obchodni-udaje">Obchodné údaje</a>'],
  ['<h3>Kontakt</h3>', '<h3>Kontakt</h3>'],
  ['<h3>Další dokumenty</h3>', '<h3>Ďalšie dokumenty</h3>'],
  ['<h3>Zpět na web</h3>', '<h3>Späť na web</h3>'],
  ['<a class="btn primary" href="/">www.zenvoria.cz</a>', '<a class="btn primary" href="/">www.zenvoria.sk</a>'],
  ['<div class="wrap">© 2026 ZENVORIA · Péče s lidskostí.</div>', '<div class="wrap">© 2026 ZENVORIA · Starostlivosť s ľudskosťou.</div>'],
  // ochrana-osobnich-udaju.html (GDPR)
  ['<title>Ochrana osobních údajů | ZENVORIA</title>', '<title>Ochrana osobných údajov | ZENVORIA</title>'],
  ['<meta name="description" content="Informace o zpracování osobních údajů v ZENVORIA.">', '<meta name="description" content="Informácie o spracovaní osobných údajov v ZENVORIA.">'],
  ['<div class="eyebrow">GDPR</div>\n        <h1>Ochrana osobních údajů</h1>\n        <p>Jak chráníme osobní údaje rodin, pečovatelek i dalších uživatelů platformy ZENVORIA</p>\n        <div class="meta">\n          <span>Naposledy aktualizováno: 13. června 2026</span>',
    '<div class="eyebrow">GDPR</div>\n        <h1>Ochrana osobných údajov</h1>\n        <p>Ako chránime osobné údaje rodín, opatrovateliek aj ďalších používateľov platformy ZENVORIA</p>\n        <div class="meta">\n          <span>Naposledy aktualizované: 13. júna 2026</span>'],
  ['<p>Ochrana vašich osobních údajů je pro ZENVORIA prioritou. Tento dokument popisuje, jaké údaje zpracováváme, za jakým účelem a jaká práva v souvislosti s nimi máte.</p>',
    '<p>Ochrana vašich osobných údajov je pre ZENVORIA prioritou. Tento dokument popisuje, aké údaje spracúvame, na aký účel a aké práva v súvislosti s nimi máte.</p>'],
  ['<h2>1. Správce údajů</h2>', '<h2>1. Prevádzkovateľ údajov</h2>'],
  ['<p>Správcem osobních údajů je {{CONTACT_NAME}}, IČO {{CONTACT_ICO}}, se sídlem {{CONTACT_ADDRESS}}. Kontakt: <a href="mailto:miklasova@zenvoria.cz">miklasova@zenvoria.cz</a>, tel. <a href="tel:{{CONTACT_PHONE_TEL}}">{{CONTACT_PHONE}}</a>.</p>',
    '<p>Prevádzkovateľom osobných údajov je {{CONTACT_NAME}}, IČO {{CONTACT_ICO}}, so sídlom {{CONTACT_ADDRESS}}. Kontakt: <a href="mailto:miklasova@zenvoria.cz">miklasova@zenvoria.cz</a>, tel. <a href="tel:{{CONTACT_PHONE_TEL}}">{{CONTACT_PHONE}}</a>.</p>'],
  ['<h2>2. Jaké údaje zpracováváme</h2>', '<h2>2. Aké údaje spracúvame</h2>'],
  ['<li>Identifikační a kontaktní údaje.</li>', '<li>Identifikačné a kontaktné údaje.</li>'],
  ['<li>Údaje o objednávkách péče a komunikaci v aplikaci.</li>', '<li>Údaje o objednávkach starostlivosti a komunikácii v aplikácii.</li>'],
  ['<li>U pečovatelek údaje nutné k ověření profilu.</li>', '<li>Pri opatrovateľkách údaje nutné na overenie profilu.</li>'],
  ['<li>Technické údaje související s provozem služby.</li>', '<li>Technické údaje súvisiace s prevádzkou služby.</li>'],
  ['<h2>3. Účel a právní základ</h2>', '<h2>3. Účel a právny základ</h2>'],
  ['<li>Poskytování služby a plnění smlouvy.</li>', '<li>Poskytovanie služby a plnenie zmluvy.</li>'],
  ['<li>Plnění zákonných povinností.</li>', '<li>Plnenie zákonných povinností.</li>'],
  ['<li>Oprávněný zájem – bezpečnost a zlepšování služby.</li>', '<li>Oprávnený záujem – bezpečnosť a zlepšovanie služby.</li>'],
  ['<li>Marketing pouze na základě souhlasu.</li>', '<li>Marketing len na základe súhlasu.</li>'],
  ['<h2>4. Doba uchování</h2>', '<h2>4. Doba uchovávania</h2>'],
  ['<p>Údaje uchováváme po dobu trvání smluvního vztahu a následně po dobu vyžadovanou právními předpisy nebo legitimním zájmem provozovatele.</p>',
    '<p>Údaje uchovávame počas trvania zmluvného vzťahu a následne po dobu vyžadovanú právnymi predpismi alebo oprávneným záujmom prevádzkovateľa.</p>'],
  ['<p>Máte právo na přístup k údajům, jejich opravu či výmaz, omezení zpracování, přenositelnost, vznesení námitky a podání stížnosti u Úřadu pro ochranu osobních údajů.</p>',
    '<p>Máte právo na prístup k údajom, ich opravu či výmaz, obmedzenie spracovania, prenosnosť, vznesenie námietky a podanie sťažnosti na Úrade na ochranu osobných údajov SR (<a href="https://www.dataprotection.gov.sk" target="_blank" rel="noopener">dataprotection.gov.sk</a>).</p>'],
  ['<p>S dotazy ke zpracování osobních údajů se obraťte na <a href="mailto:miklasova@zenvoria.cz">miklasova@zenvoria.cz</a>.</p>',
    '<p>S otázkami k spracovaniu osobných údajov sa obráťte na <a href="mailto:miklasova@zenvoria.cz">miklasova@zenvoria.cz</a>.</p>'],
  ['<a class="btn" href="/obchodni-podminky">Podmínky</a>\n            <a class="btn" href="/zasady-cookies">Cookies</a>\n            <a class="btn" href="/obchodni-udaje">Obchodní údaje</a>',
    '<a class="btn" href="/obchodni-podminky">Podmienky</a>\n            <a class="btn" href="/zasady-cookies">Cookies</a>\n            <a class="btn" href="/obchodni-udaje">Obchodné údaje</a>'],
  // obchodni-podminky.html
  ['<title>Obchodní podmínky | ZENVORIA</title>', '<title>Obchodné podmienky | ZENVORIA</title>'],
  ['<meta name="description" content="Obchodní podmínky platformy ZENVORIA.">', '<meta name="description" content="Obchodné podmienky platformy ZENVORIA.">'],
  ['<div class="eyebrow">Právní dokument</div>\n        <h1>Obchodní podmínky</h1>\n        <p>Pravidla používání platformy ZENVORIA, registrace, objednávek péče a odpovědnosti jednotlivých stran.</p>\n        <div class="meta">\n          <span>Účinné od: 13. června 2026</span>',
    '<div class="eyebrow">Právny dokument</div>\n        <h1>Obchodné podmienky</h1>\n        <p>Pravidlá používania platformy ZENVORIA, registrácie, objednávok starostlivosti a zodpovednosti jednotlivých strán.</p>\n        <div class="meta">\n          <span>Účinné od: 13. júna 2026</span>'],
  ['<p>Tyto obchodní podmínky upravují práva a povinnosti při užívání platformy ZENVORIA, která zprostředkovává spojení mezi rodinami a ověřenými pečovatelkami.</p>',
    '<p>Tieto obchodné podmienky upravujú práva a povinnosti pri používaní platformy ZENVORIA, ktorá sprostredkúva spojenie medzi rodinami a overenými opatrovateľkami.</p>'],
  ['<h2>1. Úvodní ustanovení</h2>', '<h2>1. Úvodné ustanovenia</h2>'],
  ['<p>Provozovatelem platformy je {{CONTACT_NAME}}. Registrací a užíváním služby vyjadřujete souhlas s těmito podmínkami.</p>',
    '<p>Prevádzkovateľom platformy je {{CONTACT_NAME}}. Registráciou a používaním služby vyjadrujete súhlas s týmito podmienkami.</p>'],
  ['<h2>2. Vymezení pojmů</h2>', '<h2>2. Vymedzenie pojmov</h2>'],
  ['<li><b>Uživatel</b> – rodina nebo osoba poptávající péči.</li>', '<li><b>Používateľ</b> – rodina alebo osoba dopytujúca starostlivosť.</li>'],
  ['<li><b>Pečovatelka</b> – ověřený poskytovatel pečovatelských služeb.</li>', '<li><b>Opatrovateľka</b> – overený poskytovateľ opatrovateľských služieb.</li>'],
  ['<li><b>Zprostředkování</b> – propojení uživatele a pečovatelky prostřednictvím platformy.</li>', '<li><b>Sprostredkovanie</b> – prepojenie používateľa a opatrovateľky prostredníctvom platformy.</li>'],
  ...
  ['<p>Uživatel je povinen uvádět pravdivé údaje a chránit přístupové údaje ke svému účtu. Za aktivity provedené pod účtem odpovídá jeho držitel.</p>',
    '<p>Používateľ je povinný uvádzať pravdivé údaje a chrániť prístupové údaje k svojmu účtu. Za aktivity vykonané pod účtom zodpovedá jeho držiteľ.</p>'],
  ['<h2>4. Zprostředkování péče</h2>', '<h2>4. Sprostredkovanie starostlivosti</h2>'],
  ['<p>ZENVORIA poskytuje technologickou platformu pro vyhledání a objednání péče. Samotnou péči poskytuje pečovatelka, která za ni nese odpovědnost.</p>',
    '<p>ZENVORIA poskytuje technologickú platformu na vyhľadanie a objednanie starostlivosti. Samotnú starostlivosť poskytuje opatrovateľka, ktorá za ňu nesie zodpovednosť.</p>'],
  ['<p>Ceny péče jsou uvedeny u jednotlivých pečovatelek. Platby probíhají prostřednictvím zabezpečené platební brány. Platforma si může účtovat zprostředkovatelský poplatek.</p>',
    '<p>Ceny starostlivosti sú uvedené pri jednotlivých opatrovateľkách. Platby prebiehajú prostredníctvom zabezpečenej platobnej brány. Platforma si môže účtovať sprostredkovateľský poplatok.</p>'],
  ['<h2>6. Zrušení a storno</h2>', '<h2>6. Zrušenie a storno</h2>'],
  ['<p>Objednávku lze bezplatně zrušit nejpozději 24 hodin před začátkem péče. Při pozdějším zrušení může být účtován storno poplatek.</p>',
  ...
  ['<p>Tyto podmínky se řídí právním řádem České republiky. Provozovatel je oprávněn podmínky měnit; o změnách bude informovat v aplikaci.</p>',
    '<p>Tieto podmienky sa riadia právnym poriadkom Slovenskej republiky. Prevádzkovateľ je oprávnený podmienky meniť; o zmenách bude informovať v aplikácii.</p>'],
  ['<a class="btn" href="/ochrana-osobnich-udaju">GDPR</a>\n            <a class="btn" href="/zasady-cookies">Cookies</a>\n            <a class="btn" href="/obchodni-udaje">Obchodní údaje</a>',
    '<a class="btn" href="/ochrana-osobnich-udaju">GDPR</a>\n            <a class="btn" href="/zasady-cookies">Cookies</a>\n            <a class="btn" href="/obchodni-udaje">Obchodné údaje</a>'],
  // zasady-cookies.html
  ['<title>Zásady cookies | ZENVORIA</title>', '<title>Zásady cookies | ZENVORIA</title>'],
  ['<meta name="description" content="Zásady používání cookies na webu ZENVORIA.">', '<meta name="description" content="Zásady používania cookies na webe ZENVORIA.">'],
  ['<div class="eyebrow">Cookies</div>\n        <h1>Zásady používání cookies</h1>\n        <p>Přehled souborů cookies, které používáme pro fungování aplikace, nastavení a analytiku.</p>\n        <div class="meta">\n          <span>Naposledy aktualizováno: 13. června 2026</span>',
    '<div class="eyebrow">Cookies</div>\n        <h1>Zásady používania cookies</h1>\n        <p>Prehľad súborov cookies, ktoré používame na fungovanie aplikácie, nastavenia a analytiku.</p>\n        <div class="meta">\n          <span>Naposledy aktualizované: 13. júna 2026</span>'],
  ['<p>Soubory cookies nám pomáhají zajistit správné fungování aplikace a zlepšovat vaši zkušenost. Zde se dozvíte, jaké cookies používáme a jak je můžete spravovat.</p>',
    '<p>Súbory cookies nám pomáhajú zaistiť správne fungovanie aplikácie a zlepšovať vašu skúsenosť. Tu sa dozviete, aké cookies používame a ako ich môžete spravovať.</p>'],
  ['<h2>1. Co jsou cookies</h2>', '<h2>1. Čo sú cookies</h2>'],
  ['<p>Cookies jsou malé textové soubory ukládané ve vašem prohlížeči. Umožňují například zapamatovat přihlášení nebo předvolby zobrazení.</p>',
    '<p>Cookies sú malé textové súbory ukladané vo vašom prehliadači. Umožňujú napríklad zapamätať prihlásenie alebo predvoľby zobrazenia.</p>'],
  ['<h2>2. Jaké cookies používáme</h2>', '<h2>2. Aké cookies používame</h2>'],
  ['<li><b>Nezbytné</b> – zajišťují základní funkce, přihlášení a bezpečnost.</li>', '<li><b>Nevyhnutné</b> – zaisťujú základné funkcie, prihlásenie a bezpečnosť.</li>'],
  ['<li><b>Předvolby</b> – pamatují si uživatelská nastavení.</li>', '<li><b>Predvoľby</b> – pamätajú si používateľské nastavenia.</li>'],
  ['<li><b>Analytické</b> – pomáhají vyhodnocovat a zlepšovat službu.</li>', '<li><b>Analytické</b> – pomáhajú vyhodnocovať a zlepšovať službu.</li>'],
  ['<h2>3. Správa cookies</h2>', '<h2>3. Správa cookies</h2>'],
  ['<p>Nastavení cookies můžete kdykoli změnit ve svém prohlížeči, kde je lze také smazat. Omezení některých cookies může ovlivnit funkčnost aplikace.</p>',
    '<p>Nastavenia cookies môžete kedykoľvek zmeniť vo svojom prehliadači, kde ich možno aj vymazať. Obmedzenie niektorých cookies môže ovplyvniť funkčnosť aplikácie.</p>'],
  ['<h2>4. Kontakt</h2>', '<h2>4. Kontakt</h2>'],
  ['<p>S dotazy ke cookies se obraťte na <a href="mailto:miklasova@zenvoria.cz">miklasova@zenvoria.cz</a>.</p>',
    '<p>S otázkami ku cookies sa obráťte na <a href="mailto:miklasova@zenvoria.cz">miklasova@zenvoria.cz</a>.</p>'],
];
function translateStaticLegalToSk(html) {
  let out = html;
  for (const [cz, sk] of STATIC_LEGAL_SK_TRANSLATIONS) out = out.split(cz).join(sk);
  return out;
}
app.get(Object.keys(STATIC_LEGAL_PAGES), h(async (req, res) => {
  ...
  if (countryForReq(req) === 'sk') html = translateStaticLegalToSk(html);
...
async function checkFinishedServices() {
  ...
  try {
    ...
    for (const o of rows || []) {
      ...
      if (o.family_email) {
        const famUser = await findUserByEmail(o.family_email);
        await notifyMail({ to: o.family_email, category: 'reminders', ...serviceDoneCheckMail({ familyName: o.fam_name, order: o, caregiverName, country: famUser && famUser.country }) });
      }
...
async function sendUpcomingReminders() {
  ...
  try {
    ...
    for (const o of rows || []) {
      ...
      let caregiverName = '', caregiverEmail = '', caregiverCountry = null;
      if (o.cid != null) {
        const cgs = await restSelect(T.caregivers, `id=eq.${o.cid}&select=name,email,country&limit=1`);
        const cg = cgs && cgs[0];
        caregiverName = (cg && cg.name) || '';
        caregiverEmail = (cg && cg.email) || '';
        caregiverCountry = cg && cg.country;
      }
      if (o.family_email) {
        const famUser = await findUserByEmail(o.family_email);
        await notifyMail({ to: o.family_email, category: 'reminders', ...upcomingOrderReminderMail({ name: o.fam_name, order: o, counterpartName: caregiverName, forCaregiver: false, country: famUser && famUser.country }) });
      }
      if (caregiverEmail) {
        await notifyMail({ to: caregiverEmail, category: 'reminders', ...upcomingOrderReminderMail({ name: caregiverName, order: o, counterpartName: o.fam_name, forCaregiver: true, country: caregiverCountry }) });
      }
...
async function sendCertExpiryReminders() {
  ...
  try {
    ...
    for (const v of rows || []) {
      ...
      const certUser = await findUserByEmail(v.email);
      await notifyMail({ to: v.email, category: 'reminders', ...certExpiryReminderMail({ name: v.name, certs: expiring, country: certUser && certUser.country }) });
