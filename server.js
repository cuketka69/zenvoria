...
app.post('/api/admin/cleanup-test-data', requireRole('admin'), h(async (_req, res) => {
  const cleaned = {
    orders: 0,
    requests: 0,
    schedule: 0,
  ...
  // Remove the runtime test booking and its derived records.
  const testOrders = await restSelect(
    T.orders,
    `select=oid&family_email=eq.${encodeURIComponent('demo@zenvoria.cz')}` +
      `&cid=eq.1&service=eq.osobni&date=eq.2026-06-12&time=eq.10:00` +
      `&addr=eq.${encodeURIComponent('Veleslavínská 123, Praha 6')}`
  );
  for (const order of testOrders || []) {
    await restDelete(T.orders, `oid=eq.${Number(order.oid)}`);
    await restDelete(T.requests, `oid=eq.${Number(order.oid)}`);
    cleaned.orders += 1;
  }

  const scheduleRows = await restSelect(
    T.schedule,
    `select=id&cid=eq.1&fam=eq.${encodeURIComponent('Marie Horáková')}` +
      `&service=eq.osobni&date=eq.2026-06-12&time=eq.10:00`
  );
  for (const row of scheduleRows || []) {
    await restDelete(T.schedule, `id=eq.${Number(row.id)}`);
    cleaned.schedule += 1;
  }

  // Remove the test chat message and its generated reply from the Jana conversation only.
  ...
  // Remove the admin test broadcast sent during runtime verification.
  const testBroadcasts = await restSelect(
    T.broadcasts,
    `select=id&text=eq.${encodeURIComponent('Testovací oznámení po nasazení backendu.')}`
  );
  for (const row of testBroadcasts || []) {
    await restDelete(T.broadcasts, `id=eq.${Number(row.id)}`);
    cleaned.broadcasts += 1;
  }
